...
public sealed class VehicleRegistrationController : ControllerBase
{
    ...
    public async Task<ActionResult<VehicleRegContextDto>> GetContext([FromQuery] long? passageId, [FromQuery] string? plateNorm, CancellationToken ct)
    {
        ...
        ctx.Tariffs = await _db.Tariffs.AsNoTracking()
            ...
            .OrderBy(x => x.Name)
            .Select(x => new TariffItemDto { Id = x.Id, Name = x.Name, Cost = x.Cost })
            .ToListAsync(ct);
        ...
        ctx.Statuses = await _db.WatchlistTypes.AsNoTracking()
            .Where(x => x.IsActive)
            .OrderBy(x => x.Name)
            .Select(x => new StatusItemDto { Code = x.Code, Name = x.Name })
            .ToListAsync(ct);
        ...
        else
        {
            ...
            if (activePlace != null)
            {
                ...
                ctx.RemainingPeriod = BuildRemainingText(activePlace.PaidUntil, activePlace.PausedAt, activePlace.Status);
                ctx.StateKind = GetStateKind(watch, activePlace.Status, activePlace.PaidUntil, activePlace.Tariff.GracePeriodDays);
                ctx.StateLabel = BuildStateLabel(activePlace.Status, activePlace.PaidUntil, activePlace.PausedAt);
            }
            else
            {
                ...
                ctx.StateKind = GetStateKind(watch, lastPlace?.Status, lastPlace?.PaidUntil, lastPlace?.Tariff.GracePeriodDays ?? 0);
                ctx.StateLabel = lastPlace == null ? "Есть в базе" : BuildStateLabel(lastPlace.Status, lastPlace.PaidUntil, lastPlace.PausedAt);
            }
            ...
            ctx.Payments = await _db.Payments.AsNoTracking()
                ...
                .Select(p => new PaymentRowDto
                {
                    ...
                    Tariff = p.ContractServiceId != null
                        ...
                        : p.ContractPlace != null ? p.ContractPlace.Tariff.Name : null,
                    Amount = p.Amount
                })
                .ToListAsync(ct);
        }
    ...
    public async Task<ActionResult> Save([FromBody] VehicleRegSaveDto dto, CancellationToken ct)
    {
        ...
        if (dto.OwnerId.HasValue)
        {
            ...
            var existingLinks = await _db.VehicleOwners.Where(x => x.VehicleId == vehicle.Id).ToListAsync(ct);
            var existingSameOwner = existingLinks.FirstOrDefault(x => x.OwnerId == ownerId);

            if (existingSameOwner == null)
            {
                _db.VehicleOwners.RemoveRange(existingLinks);
                _db.VehicleOwners.Add(new VehicleOwnerRow { VehicleId = vehicle.Id, OwnerId = ownerId, IsPayer = true });
            }
            else
            {
        ...
        {
            var statusType = await _db.WatchlistTypes.FirstOrDefaultAsync(x => x.Code == statusCode && x.IsActive, ct);
            if (statusType == null)
                return BadRequest($"Unknown status: {statusCode}");

            var existingWatch = await _db.Watchlist.FirstOrDefaultAsync(x => x.PlateNorm == dto.PlateNorm, ct);
    ...
    public async Task<ActionResult> ClearOwner([FromBody] PlateActionDto dto, CancellationToken ct)
    {
        ...
        var vehicle = await _db.Vehicles.FirstOrDefaultAsync(v => v.PlateNorm == plate && v.IsActive, ct);
        if (vehicle == null)
            return NotFound("Автомобиль не найден.");

        var links = await _db.VehicleOwners.Where(x => x.VehicleId == vehicle.Id).ToListAsync(ct);
    ...
    public async Task<ActionResult> Depart([FromBody] PlateActionDto dto, CancellationToken ct)
    {
        ...
        var nowUtc = DateTimeOffset.UtcNow;
        active.ContractPlace.PauseBalanceDays = CalcRemainingDaysMinusOne(active.ContractPlace.PaidUntil, nowUtc);
    ...
    public async Task<ActionResult> PauseToggle([FromBody] PlateActionDto dto, CancellationToken ct)
    {
        ...
        {
            if (!active.ContractPlace.PausedAt.HasValue || active.ContractPlace.PausedAt.Value.AddMonths(3) <= nowUtc)
            {
                active.ContractPlace.Status = "Closed";
        ...
        if (!active.ContractPlace.PaidUntil.HasValue || active.ContractPlace.PaidUntil.Value <= nowUtc)
    ...
    private async Task<ActiveContractInfo?> FindActivePlaceByPlateAsync(string? plateNorm, bool track, CancellationToken ct)
    {
        var plate = (plateNorm ?? "").Trim().ToUpperInvariant();
        if (string.IsNullOrWhiteSpace(plate))
            return null;

        var vehicle = await _db.Vehicles.AsNoTracking().FirstOrDefaultAsync(v => v.PlateNorm == plate && v.IsActive, ct);
        if (vehicle == null)
            return null;
        ...
        var cp = await query
            ...
            .ThenByDescending(cp => cp.PaidUntil)
            .FirstOrDefaultAsync(ct);
        return cp == null ? null : new ActiveContractInfo(cp.Contract, cp, cp.Tariff);
    }
    ...
    private async Task<long> GetOrCreateAnonymousOwnerIdAsync(CancellationToken ct)
    {
        ...
        var owner = new OwnerRow { Surname = anonymousSurname, FirstName = "", IsActive = true };
        _db.Owners.Add(owner);
        await _db.SaveChangesAsync(ct);
        return owner.Id;
    }

    private static int CalcRemainingDaysMinusOne(DateTimeOffset? paidUntil, DateTimeOffset nowUtc)
    {
    ...
    private static string BuildRemainingText(DateTimeOffset? paidUntil, DateTimeOffset? pausedAt, string status)
    {
    ...
    private static string BuildStateLabel(string status, DateTimeOffset? paidUntil, DateTimeOffset? pausedAt)
    {
        if (status == "Paused" && pausedAt.HasValue)
            return $"Пауза до {pausedAt.Value.AddMonths(3).LocalDateTime:dd.MM.yyyy}";
    ...
    {
        var fileName = string.IsNullOrWhiteSpace(p.JpegPath) ? null : Path.GetFileName(p.JpegPath);
    ...
    private sealed record ActiveContractInfo(ContractRow Contract, ContractPlaceRow ContractPlace, TariffRow Tariff);
}
